      ...
      <div className="mt-8 grid w-full max-w-xl grid-cols-1 gap-3 sm:grid-cols-2">
        {suggestions.map((s) => (
          <button
            ...
            className="focus-ring group flex flex-col items-start gap-1.5 rounded-xl border border-[var(--border)] bg-[var(--bg)] p-4 text-left hover:border-accent/40 hover:bg-[var(--bg-hover)] transition-all"
          >
            <div className="flex items-center gap-2 text-body-sm font-medium text-[var(--text-primary)] group-hover:text-accent transition-colors">
              <s.icon size={16} className="text-accent flex-shrink-0" />
              <span>{s.label}</span>
            </div>
